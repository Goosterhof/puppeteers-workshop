#!/usr/bin/env bash
# The Prompter's Box — the booth that feeds lines to the performers
# UI: http://localhost:7900  (reachable from the Windows browser; WSL2 forwards localhost)
# Needs: Ollama on :11434 for the Forge; ComfyUI on :8188 for Face Shop cues.
# Stage cues spawn wgp.py --process headlessly — keep the full Wan2GP UI closed.
#
# Since the Kiln Room (#00062) the booth boots on the ComfyUI machine venv:
# server.py imports kiln/turntable (numpy, trimesh, pyrender — see
# prompter-box/requirements.txt), so plain python3 can no longer carry it.
# One venv, already on the floor — no venv of our own, per the conventions.
set -euo pipefail
cd "$(dirname "$0")/prompter-box"
exec ../ComfyUI/.venv/bin/python server.py "$@"
