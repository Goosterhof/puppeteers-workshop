// The dist-drift guard (#00063 §1A): the committed static/ bundle must
...
const out = execSync('git status --porcelain -- ../static', {encoding: 'utf8'}).trim();
if (out) {
    console.error(`dist drift — static/ does not match a fresh build:\n${out}\n` +
        'Rebuild and commit the bundle alongside the source change.');
    process.exit(1);
}
console.log('static/ matches a fresh build — no drift.');
