...
bench or the blueprint — merged with a MERGE COMMIT (Decision 025; see The
merge path below). The bench's standing duty is
...
4. **The investor's merge is the authorization** — a MERGE COMMIT (Decision
   025, 2026-08-08: squash and rebase are disabled repo-side, so the wrong
   button does not exist). The PR branch tip stays an ancestor of `main`
   forever — a parent gitlink pointed at it can never dangle, and the old
   squash-era re-point RITUAL (with its silent orphaned-gitlink window) is
   retired. If a merge is ever refused with "Merge commits are not allowed,"
   the culprit is `required_linear_history` in branch protection — read the
   decision file before touching settings.
5. The parent still syncs its gitlink to the `main` tip afterwards as an
   ordinary `sync(workshop): ...` commit — bookkeeping now, not rescue.
